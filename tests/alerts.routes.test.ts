...
describe('alerts subscribe/unsubscribe routes', () => {
  ...
  it('subscribe returns ok and dev tokens', async () => {
    ...
    expect(data.tokens[0].channel).toBe('email');
    // token doc keyed by token; subscriber row created but not opted in
    expect(col('alerts_confirmations').has(data.tokens[0].token)).toBe(true);
    expect(subscriberByEmail('a@b.com')?.email_opt_in).toBe(false);
  });
  ...
  it('subscribe re-uses the existing subscriber row (upsert on email)', async () => {
    ...
    expect(updateSpy).toHaveBeenCalledWith('alerts_subscribers', 'existing-uuid', expect.anything());
  });
  ...
  it('unsubscribe returns tokens', async () => {
    ...
    expect(data.ok).toBe(true);
    expect(Array.isArray(data.tokens)).toBe(true);
    expect(col('alerts_confirmations').get(data.tokens[0].token)?.action).toBe('unsubscribe');
  });
